...
/**
 * Arrange processed products in a grid layout.
 * Supports dynamic cols/rows and canvas dimensions for orientation support.
 */
export function arrangeInGrid(
    processedProducts: (MoodboardProduct & { imgElement: HTMLImageElement })[],
    canvasWidth: number = A4_WIDTH,
    canvasHeight: number = A4_HEIGHT,
    cols: number = 2,
    rows: number = 2
) {
    const padding = GRID_PADDING
    const colWidth = (canvasWidth - padding * (cols + 1)) / cols
    const rowHeight = (canvasHeight - padding * (rows + 1)) / rows
    ...
    return processedProducts.map((p, i) => {
        ...
        return {
            ...p,
            x: padding + col * (colWidth + padding) + (cellW - w) / 2,
